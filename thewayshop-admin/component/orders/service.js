...
exports.getAllRecord = (page)=>{
    return pool.query(
        `SELECT orders.id,user_id,fullname,email,address,detail_address,payment,total,state as state_id,order_state.description as state,TO_CHAR(create_date::date, 'dd-mm-yyyy') as create_date,cancel FROM orders,order_state WHERE state=order_state.id
